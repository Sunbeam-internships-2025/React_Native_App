...
const API_URL = Platform.OS === 'android' ? '' : 'http://localhost:1111';
...
const StudentLogin = ({ navigation, setStudent, setRole }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.data.status === 'Success') {
        const studentData = response.data.data;
        const student = { id: studentData.user_id, name: studentData.first_name + ' ' + studentData.last_name, email: studentData.email };
        await AsyncStorage.setItem('studentName', student.name);
        await AsyncStorage.setItem('studentId', student.id.toString());
        await AsyncStorage.setItem('role', 'Student');
        setStudent(student);
        ...
        Toast.show({ type: 'error', text1: response.data.message || 'Invalid email/password' });
      }
    } catch (err) {
      console.error(err.message);
      Toast.show({ type: 'error', text1: 'Network Error', text2: 'Cannot connect to server.' });
    } finally {
      setLoading(false);
    }
  };
...
const styles = StyleSheet.create({
  container: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20, backgroundColor: '#f2f6fc' },
  title: { fontSize: 28, fontWeight: '700', marginBottom: 40 },
  input: { width: '90%', maxWidth: 400, backgroundColor: '#fff', padding: 15, marginBottom: 20, borderRadius: 12, borderWidth: 1, borderColor: '#e0e6ed' },
  button: { width: '90%', maxWidth: 400, backgroundColor: '#3b82f6', padding: 15, borderRadius: 12, alignItems: 'center', marginTop: 10 },
